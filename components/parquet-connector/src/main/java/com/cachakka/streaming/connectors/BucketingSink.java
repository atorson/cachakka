...
public class BucketingSink<T>
     ...
     */
    private transient BucketingSink.State<T> state;
    ...
    public void initializeState(FunctionInitializationContext context) throws Exception {
        ...
        if (context.isRestored()) {
            ...
            for (BucketingSink.State<T> recoveredState : restoredBucketStates.get()) {
    ...
    public void close() throws Exception {
        if (state != null) {
            for (Map.Entry<String, BucketingSink.BucketState<T>> entry : state.bucketStates.entrySet()) {
    ...
    public void invoke(T value) throws Exception {
        ...
        BucketingSink.BucketState<T> bucketState = state.getBucketState(bucketPath);
        if (bucketState == null) {
            bucketState = new BucketingSink.BucketState<>(currentProcessingTime);
     ...
     * </ol>
     */
    private boolean shouldRoll(BucketingSink.BucketState<T> bucketState) throws IOException {
     ...
     */
    private void openNewPartFile(Path bucketPath, BucketingSink.BucketState<T> bucketState) throws Exception {
     ...
     */
    private void closeCurrentPartFile(BucketingSink.BucketState<T> bucketState) throws Exception {
        if (bucketState.isWriterOpen) {
     ...
     */
    private Method reflectTruncate(FileSystem fs) { return null;
       /* Method m = null;
        if (fs != null) {
    ...
    public void notifyCheckpointComplete(long checkpointId) throws Exception {
        synchronized (state.bucketStates) {
            ...
            Iterator<Map.Entry<String, BucketingSink.BucketState<T>>> bucketStatesIt = state.bucketStates.entrySet().iterator();
    ...
    public void snapshotState(FunctionSnapshotContext context) throws Exception {
        ...
        synchronized (state.bucketStates) {
            ...
            for (Map.Entry<String, BucketingSink.BucketState<T>> bucketStateEntry : state.bucketStates.entrySet()) {
                BucketingSink.BucketState<T> bucketState = bucketStateEntry.getValue();
    ...
    private void handleRestoredBucketState(BucketingSink.State<T> restoredState) {
        ...
        for (BucketingSink.BucketState<T> bucketState : restoredState.bucketStates.values()) {
    ...
    @VisibleForTesting
    public BucketingSink.State<T> getState() {
        return state;
    }
    ...
    static final class State<T> implements Serializable {
         ...
         */
        final Map<String, BucketingSink.BucketState<T>> bucketStates = new HashMap<>();

        void addBucketState(Path bucketPath, BucketingSink.BucketState<T> state) {
